...
| 天 | 主题 | 内容 | Java 类比锚点 |
|----|------|------|--------------|
| **Day1 (06/05 五)** | 环境 + Python 速通 | Python 类型系统、控制流、异步 IO（asyncio）、包管理（uv/pip） | asyncio ≈ CompletableFuture；list comprehension ≈ Stream API；dict ≈ HashMap |
| **Day2 (06/08 一)** | Python 进阶 + LLM 概念 | Pydantic 数据模型、FastAPI 基础、LLM 原理（Transformer/Attention）、Token 机制 | Pydantic ≈ Lombok @Data + Bean Validation；API 调用 ≈ HTTP Client |
| **Day3 (06/09 二)** | LLM API + Agent 概念 | OpenAI Chat Completions API（DeepSeek）、流式响应（SSE）、System Prompt、Agent 架构与 ReAct 模式 | System Prompt ≈ 装配好的 Bean Definition；Agent 架构 ≈ MVC 模式 |
| **Day4 (06/10 三)** | 项目实战 + 完善 | 实现「Hello Agent」：多轮对话、上下文管理、错误重试、Rate Limit、Token 成本统计 | 上下文管理 ≈ Session 管理；Rate Limit ≈ 熔断降级 |
...
| 天 | 主题 | 内容 | Java 类比锚点 |
|----|------|------|--------------|
| **Day6 (06/12 五)** | Prompt 工程基础 | System Prompt 设计模式、Chain-of-Thought、Few-Shot、角色扮演 Prompt | Prompt 模板 ≈ JSP/Thymeleaf 模板引擎 |
| **Day7 (06/15 一)** | 结构化输出 + Tool Use | JSON Mode、Function Calling 格式、Pydantic 解析校验、Tool 注册与调度 | Function Calling Schema ≈ Swagger/OpenAPI；Tool 注册 ≈ Spring Bean |
| **Day8 (06/16 二)** | MCP 协议入门 + Server 实战 | MCP 架构（Server/Client/Transport）、三原语、FastMCP SDK、构建自定义 MCP Server | MCP ≈ SPI 机制 + 服务发现 |
| **Day9 (06/17 三)** | 项目实战2（上） | 多工具 Agent 实现：天气查询 + 数据库查询 + 文件操作、Tool 注册、调用链路 | 工具链编排 ≈ Chain of Responsibility |
...
| 天 | 主题 | 内容 | Java 类比锚点 |
|----|------|------|--------------|
| **Day11 (06/19 五)** | Embedding + 向量数据库 | 文本向量化、相似度计算、ChromaDB 入门、文档切分（Chunking）、检索流程 | Embedding ≈ Lucene 倒排索引；向量数据库 ≈ ES kNN |
| **Day12 (06/22 一)** | RAG 架构 + LangChain | Indexing → Retrieval → Generation、Chains/LCEL、Prompt Templates | RAG Pipeline ≈ ETL 流程；LangChain ≈ Spring 生态 |
| **Day13 (06/23 二)** | LangGraph 入门 | StateGraph 概念、Node/Edge 定义、条件路由、Checkpoint 机制 | StateGraph ≈ 工作流引擎（Flowable BPMN）；Checkpoint ≈ Saga |
| **Day14 (06/24 三)** | 项目实战3（上） | RAG 知识库 Agent：文档上传 → 向量索引 → 智能问答 | — |
...
| 天 | 主题 | 内容 | Java 类比锚点 |
|----|------|------|--------------|
| **Day16 (06/26 五)** | 多 Agent 模式 + 生产化（上） | Manager-Worker、流水线、对等协作、Human-in-the-Loop、日志与链路追踪 | 多 Agent ≈ 微服务编排；可观测性 ≈ SkyWalking/Prometheus |
| **Day17 (06/29 一)** | 生产化（下）+ 项目设计 | Agent 安全（注入防御、权限）、评估体系、综合项目架构设计 | 安全 ≈ Spring Security Filter Chain |
| **Day18 (06/30 二)** | 综合项目实现（上） | Agent 注册中心 + 任务分发 + 结果聚合 | — |
| **Day19 (07/01 三)** | 综合项目实现（下）+ 文档 | 人机协同节点、代码完善、README、架构图、运行演示 | — |
