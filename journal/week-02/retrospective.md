...
- 面经 4/15（题1-4：Agent vs ChatBot、Function Calling、MCP协议、LangGraph StateGraph）
- Java 面试备战方案完成（`projects/03-java-interview/README.md`），待启动实战练习
...
### 1. 面经和 Java 备战必须化固为习惯

**问题**：面经 4/15（欠 6 道），Java 备战方案写了但一天没动。这两项是学习计划的一部分，不能因为「忙着写代码」就推后。

**调整**：面经每天 2 道 + Java 备战每天 45min，作为开机第一件事，不推到晚段。
...
**问题**：代码量够了（Python 14 文件 + Java 12 文件），但面试官不会跑你的代码。

**调整**：Agent 网关的 README 里放架构图缩略链接 + 一行启动命令，让面试官扫一眼就知道你做了什么。
...
| 面试准备 | 5/10 | 面经欠债，Java 备战未启动。代码写得再多，讲不出来等于白写 |
| 习惯养成 | 7/10 | 每日编码习惯稳固，但面经+Java 备战未固化进日常节奏 |
