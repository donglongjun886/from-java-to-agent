...
**P0 必考**：[一、项目经历 & 技术选型](#一项目经历--技术选型p0) | [二、Java并发底层原理](#二java-并发底层原理p0) | [三、Spring Framework 深度](#三spring-framework-深度原理p0) | [四、微服务体系核心](#四微服务体系核心p0)

**P1 高频**：[五、中台架构设计](#五中台架构设计p1-补充) | [六、MySQL 深度](#六mysql-深度p1) | [七、Redis 深度](#七redis-深度p1) | [八、消息队列深度](#八消息队列深度p1) | [九、IO模型 & Netty](#九io-模型--nettyp1) | [十、分布式理论](#十分布式理论p1) | [十一、系统设计 & 算法](#十一系统设计--算法p1)

**P2 扩展**：[十二、JVM & 故障排查](#十二jvm--故障排查p2) | [十三、操作系统 & 网络](#十三操作系统--网络基础p2) | [十四~十九、速览](#十四设计模式在框架中的应用p2)
...
**Orderly（顺序消费）**：一个 MessageQueue 同一时间只能被一个消费者线程独占。`实际并发 = min(Queue数, 实例数)`。扩容先扩 Queue 数，实例数 ≤ Queue 数。

**Concurrently（并发消费）**：同一个 Queue 的不同批次可以被线程池中不同线程并发处理。因此增加线程数可以提升吞吐（尤其 IO 密集型场景如 RPC/DB 操作），不受 Queue 数严格限制。`实际并发 ≈ 线程池大小`（上限由 pullThresholdForQueue 控制，默认 1000 条）。扩容策略：先加线程数 → 线程数已够仍堆积则加 Queue 数 → 再加实例（实例 ≤ Queue 数）。
