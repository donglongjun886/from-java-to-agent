...
> 技术细节见前面 MySQL/JVM/Redis 章节，本节只讲排查框架。

### 20.1 排查方法论

**四步闭环**：建立基线 → 瓶颈定位 → 分层优化 → 验证闭环，绝不靠猜。

- **建立基线**：P50/P90/P99、QPS、CPU/内存/GC、慢SQL TOP10、缓存命中率。说"P99从200ms恶化到2.3s，20:00-21:00高峰"，不说"这个接口慢"
- **瓶颈定位**：全局（CPU/内存/GC）→ 链路（trace/日志）→ 代码（参数/返回值），逐层缩小范围
- **分层优化**：客户端(节流/CDN) → 网关(限流/鉴权) → 应用(异步/并行/缓存) → 中间件(索引/大key) → 基础设施(SSD/扩容)
- **验证闭环**：前后对比基线指标 → 预发1.5倍压测 → 灰度10%→100%

### 20.2 接口慢的排查思路

**五层漏斗**：网络 → 应用 → 中间件 → DB/缓存 → 下游。

- **网络（30秒排除）**：ping/curl 测 RTT，通常50ms以内
- **应用层（70%根因）**：Arthas trace 从 Controller 入口逐层追踪。重点关注 for 循环中的 RPC/DB 调用（N+1）、大对象序列化、正则回溯
- **中间件**：连接池打满（druid监控台）、线程池队列积压（activeCount/queueSize）
- **DB/缓存**：`SHOW FULL PROCESSLIST` 看 State 列、Redis `slowlog get 100`
- **下游**：调用链看耗时占比，超时配置2-3s足够

**三个不要**：不要上来加缓存、不要上来改JVM参数、不要只看平均值（P99才有意义）。

### 20.3 压测与容量评估

- **压测模型**：基于线上真实流量建模，流量漏斗比例要对，80%流量打20%热点
- **数据隔离**：流量染色全链路透传（HTTP header → Dubbo → MQ → 异步线程），影子表/影子库防污染
- **递进施压**：10%→50%→80%→100%→摸高→100%稳定跑30分钟
- **容量公式**：集群容量 = 单机拐点QPS × 机器数 × 0.8，实际达60%触发扩容告警
...
**选型定调**：资金扣款用 TCC（强一致），长链路清算用 Saga（最终一致），非关键异步用本地消息表。不要在高并发资金链路用 Seata AT 模式。TCC 三陷阱（空回滚/悬挂/幂等）详见第四章分布式事务。
...
**可讲案例**：大促期间用户连续点击两次「确认支付」，前端按钮防重被网络抖动绕过，后端没有幂等导致重复扣款。修复方案：订单号+版本号唯一索引 + 状态机 WHERE status='待支付' + 前端 Token 机制作第一道防线。
...
**可讲案例**：电商平台和支付宝每日对账。凌晨拉取支付宝对账单（CSV），流式读取避免 OOM。统一成「渠道单号+实付金额+交易时间」三元组，按渠道单号哈希分片 + 排序 + 双指针流式比对。差异主要由日切窗口造成，T+2 自动消失。
...
**可讲案例**：营销活动从"满100减10"逐步叠加互斥规则、限用次数、新用户限制，贫血模型三个 Service 各自校验经常遗漏。重构为充血：`Activity` 聚合根封装 `canApplyTo(order)`，内部逐个调用 `Rule` 值对象的 `validate()`。新增规则只需新增 Rule 实现类并注册。

### 22.4 聚合设计原则和常见陷阱

**核心原则**：聚合要尽量小（不超过5个对象），通过 ID 引用其他聚合；聚合内强一致，聚合间最终一致。

**常见陷阱**：
- **大聚合**：订单+订单项+支付+物流+发票全放一个聚合，改物流状态要锁整个订单
- **双向引用**：用户聚合放订单ID列表，订单又放用户对象引用
- **用数据库自增ID作聚合标识**：聚合ID应该是业务标识（如订单号）

### 22.5 DDD落地到代码结构

**COLA四层**：Adapter（参数校验/格式转换）→ Application（用例编排/事务管理）→ Domain（业务规则，不依赖任何层）→ Infrastructure（Repository实现/DB/缓存/消息）。

**关键实践**：仓储接口在领域层、实现在基础设施层；领域事件在聚合根记录、应用层发布；读操作可绕过领域层（CQRS：写走完整链路，读直接 Adapter→Infra）；不要一上来全套 DDD，用分级策略。
...
**Java 类比**：Tool 注册≈SPI/Spring Bean 注入；Planning≈Flowable 工作流；Memory≈Session+Redis。详见 23.6 转型优势。

### 23.2 Function Calling / Tool Use 原理
...
**关键细节**：LLM 不执行任何代码，只输出 JSON，执行是 Agent 框架的职责（经典陷阱题）。多轮调用是常态——"查订单并退款"实际链路可能有 3-5 轮 LLM 调用。

### 23.3 MCP 协议（Model Context Protocol）
...
**电商场景**：客服知识库通过 RAG 实时检索注入 Prompt，知识更新从周级缩短至分钟级（改文档即生效，不需要重新训练模型）。

### 23.5 Agent 在电商中的落地场景

| 场景 | Agent 角色 | 核心价值 |
|------|-----------|----------|
| 智能客服 | 售前咨询+售后退款+FAQ，调用真实业务接口 | 人工坐席成本大幅降低 |
| 自然语言问数 | 自然语言→SQL→查库→生成图表+解读 | 运营自己问数，不用找DBA |
| 营销文案 | 根据商品属性+活动规则批量生成 | 从0→60分，人做60→90打磨 |
| 代码生成 | 需求→写代码→跑测试→提交PR | 提效，但仍需人工Review |

### 23.6 为什么 Java 工程师转型 Agent 有优势（面试核心卖点）
...
**面试定调话术**：「我转型 Agent 不是从零开始。多年后端经验沉淀的系统设计能力在 Agent 场景里直接可迁移。我的差异化价值是：既能理解 AI 的能力边界，又能把 AI 能力工程化地嵌入业务系统。团队需要的是打通「业务系统」和「智能算法」之间的桥梁，这就是我的定位。」

**避坑提醒**：不要说自己「放弃了 Java」，而是说「在 Java 工程基础上增加了 AI 能力层」。后端能力在 Agent 项目里仍然是必需的——接口、数据库、缓存、消息队列、部署运维。Agent 不等于替代后端，Agent 需要后端。
---
## 二十四、高并发系统设计：限流策略与串联框架（P1补充）
...
**工程实践**：
- 单机限流：Guava RateLimiter 或 Sentinel
- 分布式限流：Redis + Lua 脚本原子执行
- 分级限流：按用户/IP/接口/服务逐级收紧，层层收口
- 配合熔断降级：限流触发后返回降级响应，不能让用户看到 500

### 24.2 高并发系统的端到端串联框架

当面试官问「你的系统怎么扛高并发的？」，用纵深防御六层回答，每层一句话，根据追问展开：

> 接入层（CDN+网关限流）→ 缓存层（Caffeine+Redis）→ 队列层（MQ削峰填谷）→ 应用层（弹性伸缩+连接池）→ 数据层（读写分离+分库分表）→ 兜底层（熔断降级+对账补偿）
